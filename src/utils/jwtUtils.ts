...
export function protect(
  ...
  return async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (
      (await request.reqData.redisClient.get(request.cookies.token)) !== null
    ) {
      return {
        error: 'UNAUTHORIZED',
      };
    ...
    if (decoded.t === USER_CLIENT) {
      const client = await new UserTable(request.reqData.pgClient).getById(
        decoded.i
      );
      ...
      const worker = await new WorkerTable(request.reqData.pgClient).getById(
        decoded.i
      );

      if (
        !worker ||
        (worker && options.role && !options.role.includes(worker.role_id))
      ) {
        return {
          error: 'FORBIDDEN',
        };
